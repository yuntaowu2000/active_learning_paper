...
class PDEModelTimeStepNAgents(PDEModelNAgents, PDEModelTimeStep):
    ...
    def _inject_t0_base(self, SV, vd):
        """Evaluate the xi / alpha networks at the ``t = min_t`` boundary slice
        and expose the results under ``*_t0`` names so the equation system can
        build a portfolio-FOC residual anchored at t=0 -- the slice that is
        actually extracted as the stationary solution.

        Only the pseudo-time column of ``SV`` is replaced by ``min_t``; the
        wealth-share columns (and therefore ``SV_share``) are unchanged, so the
        t=0 FOC chain can reuse ``SV_share`` directly.  Gradients still flow to
        the network parameters through this extra forward pass.
        """
        if self._xi_stack is None or self._alpha_stack is None:
            return
    ...
    def update_variables(self, SV, vd=None):
        """Same fused forward sweep as ``PDEModelNAgents`` plus the t=0 boundary
        network outputs, injected *before* the equation sweep so the ``*_t0``
        FOC chain (registered as ordinary equations) can consume them."""
        if vd is None:
            vd = self.variable_val_dict
        SV.requires_grad_(True)
        for i, sv_name in enumerate(self.state_variables):
            vd[sv_name] = SV[:, i:i+1]
        vd["SV"] = SV
        self._inject_t0_base(SV, vd)
        super().update_variables(SV, vd)
    ...
    def sample_rar_greedy(self):
        ...
        Free-boundary fix: plain ``sample_simplex_ts`` spreads its anchors over
        ALL pseudo-times ``t in (min_t, max_t)``, so the high-residual corner
        where a portfolio constraint binds is only refined at random t and the
        extracted ``t = min_t`` slice never gets densified -- the cap kink stays
        rounded.  We therefore split the refinement budget: half the anchors are
        the usual interior ``(x, t)`` points, half are pinned to ``t = min_t``
        (scored by the same residual, which includes the binding VI / ``hjb_t0``
        terms that spike at the kink).  This gives the extracted slice the same
        corner refinement that makes the stationary ``*_rar`` model resolve the
        free boundary.
        """
        self.set_all_model_eval()
        ...
        SV_int, l_int = self._score_pool(self.sample_simplex_ts)
        SV_t0, l_t0 = self._score_pool(self._sample_simplex_at_t0)
    ...
    *exactly* the residual used in ``gp_constraint_NN.py`` for the c-agent,
    ...
    if timestepping:
        cfg = {
            ...
            "loss_balancing": loss_balancing, "outer_loop_convergence_thres": 1e-5
        }
    ...
    if timestepping:
        ...
        eqs = [
            ...
            "sigx_active_t0=compute_sigx_n(SV_share, alpha_active_t0, sigR_t0)",
            "sigxi_active_t0=torch.bmm(xi_Jac_s_t0/(xi_active_t0.unsqueeze(-1)+1e-8), sigx_active_t0)",
...
def evaluate_validation_losses(model: PDEModel, SV_val: torch.Tensor, chunk_size: int = 1000):
    ...
    # The time-stepping models carry auxiliary loss terms that the stationary
    # models do not: the t=0 *anchors* (``*_t0`` -- ``hjbeq_hjb_t0`` and
    # ``endogeq_vi_active_t0``) re-evaluate the SAME economic residual on the
    # extracted slice, and the ``*_cond_time_boundary`` terms only fit each
    # slab's terminal condition.  Counting them in the reported HJB/total would
    # (a) double-count the HJB and FOC residuals and (b) make the cross-method
    # comparison unfair (timestep totals inflated vs. stationary).  We therefore
    # report only the shared interior economic objective: mc + vi_active +
    # pricing + the interior HJB (hjbeq_1).
    def _is_aux(k: str) -> bool:
        return k.endswith("_t0") or "cond_time_boundary" in k

    core = {k: v for k, v in components.items() if not _is_aux(k)}
    hjb   = sum(v for k, v in core.items() if k.startswith("hjbeq"))
    endog = sum(v for k, v in core.items() if k.startswith("endogeq"))
    total = sum(core.values())
    return {"hjb": hjb, "endog": endog, "total": total, "components": components}


def compute_alpha_xi_distributions(model, SV_val, chunk_size: int = 1000):
    n_total = SV_val.shape[0]
    n_chunks = (n_total + chunk_size - 1) // chunk_size
    alphas = []
    xis = []
    for c in range(n_chunks):
        ...
        model.variable_val_dict["SV"] = SV_chunk
        model.update_variables(SV_chunk)
        alphas.append(model.variable_val_dict["alpha_active"].detach().cpu().numpy())
        xis.append(model.variable_val_dict["xi_active"].detach().cpu().numpy())
        del SV_chunk
        gc.collect(); torch.cuda.empty_cache()

    gamma_vec = model.variable_val_dict["gamma_active"].cpu().reshape(-1).numpy()
    alpha_full = np.concatenate(alphas, axis=0)
    xi_full = np.concatenate(xis, axis=0)
    iter_dict = {}
    for i in range(len(gamma_vec)):
        iter_dict[f"alpha_{i+1}"] = alpha_full[:, i]
        iter_dict[f"xi_{i+1}"] = xi_full[:, i]

    idx = pd.Index(list(range(1, len(gamma_vec)+1)), name="agent_idx")
    df = pd.DataFrame(index=idx, columns=["alpha_mean", "alpha_low", "alpha_high", "alpha_std", "xi_mean", "xi_low", "xi_high", "xi_std"])
...
def plot_alpha_chat_histogram(model: PDEModelNAgents, out_dir, seed=0):
    SV_val = _sample_validation_simplex(len(model.params["gamma_active"]) - 1, n_samples=10000, seed=seed)
    generated_df = compute_alpha_xi_distributions(model, SV_val)
...
# Named validation cases (single source of truth shared by __main__ and the
# forward simulator ``gp_n_agents_simulate.py``).
CASES = {
    "sym2":        dict(n=2,  gamma=[4.0, 4.0],                 alpha_caps=None),
    "asym2":       dict(n=2,  gamma=[8.0, 4.0],                 alpha_caps=None),
    "sym2_const":  dict(n=2,  gamma=[4.0, 4.0],                 alpha_caps=[1e6, 1.8]),
    "asym2_const": dict(n=2,  gamma=[8.0, 4.0],                 alpha_caps=[1e6, 1.8]),
    "asym2_const2": dict(n=2,  gamma=[8.0, 4.0],                 alpha_caps=[1e6, 1.3]),
    "mix5":        dict(n=4,  gamma=[5.0, 4.0, 3.0, 2.0],       alpha_caps=[1e6, 1e6, 1e6, 1.8]),
    "mix20":       dict(n=20, gamma=[21 - i for i in range(20)], alpha_caps=[1e6] * 19 + [1.8]),
    "mix50":       dict(n=50, gamma=[51 - i for i in range(50)], alpha_caps=[1e6] * 49 + [1.8]),
}


def make_case(case: str):
    """Return ``(n_active, gamma_active, psi_active, alpha_caps, suffix)`` for a
    named validation case so both ``__main__`` and the simulator reconstruct
    the *identical* model configuration."""
    if case not in CASES:
        raise ValueError(f"unknown case {case!r}; choices: {list(CASES)}")
    spec = CASES[case]
    n = spec["n"]
    gamma_active = list(spec["gamma"])
    psi_active = [1.5] * n
    return n, gamma_active, psi_active, spec["alpha_caps"], case
...
if __name__ == "__main__":
    ...
    parser.add_argument("--case", choices=list(CASES), default="sym2")
    ...
    comparison_dir = f"{case_dir}/comparison"
    ...
    for name, ts, rar, lb in [(n, *CONFIGS[n]) for n in models]:
        plot_alpha_chat_histogram(models[name], model_paths[name])
